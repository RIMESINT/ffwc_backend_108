...
class SourceDDResponseV2Serializer(serializers.Serializer): 
    id = serializers.IntegerField()
    name = serializers.CharField()
    basin = serializers.CharField()
    division = serializers.CharField()
    ...
    def get_rainfall_observations(self, obj):
        # Fetch all rainfall observations related to this station (obj)
        # rainfall_observations = FfwcImdRainfallObservation.objects.filter(st=obj)
        # print(" $$$$$$$$$$$$$ obj: ", obj.id)
        # return RainfallObservationSerializer(rainfall_observations, many=True).data
        
        try: 
            queryset = FfwcImdRainfallObservation.objects.filter(st__id=obj.id) 
...
class RFObsDetailsResV2Serializer(serializers.Serializer):     
    id = serializers.IntegerField()
    rf_date = serializers.DateTimeField()   
    rainFall = serializers.DecimalField(max_digits=10, decimal_places=2)
    st = SourceDDResponseSerializer() 
...
"""
    #####################################################################
    ### START Serializer for StationListViewSetV5
    #####################################################################
"""
from app_visualization.models import StationListDDDetailsV5
class StationListDDDetailsV5Serializer(serializers.ModelSerializer):
    class Meta:
        model = StationListDDDetailsV5
        fields = '__all__'

"""
    #####################################################################
    ### END Serializer for StationListViewSetV5
    #####################################################################
"""
