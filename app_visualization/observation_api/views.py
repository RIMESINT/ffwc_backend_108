...
from app_visualization.observation_api.serializers import (
    ...
    RFObsDetailsResV2Serializer,
    
    SourceDDResponseV2Serializer,
...
"""
@method_decorator(csrf_exempt, name='dispatch') 
class StationListViewSetV2(viewsets.ViewSet):  
    permission_classes = (IsAuthenticated,)
    ...
    def station_list_dd(self, request):
        ...
        if req_serializer.is_valid():  
            try:
                queryset = FfwcIMDRFObservationStation.objects.select_related(
                    'observe_data_source'
                ).filter( 
                    observe_data_source=req_serializer.data['observe_data_source'],
                    lat__isnull=False,
                ...
                res_serializer = SourceDDResponseV2Serializer(queryset, many=True) 
                return Response(res_serializer.data, status=status.HTTP_200_OK) 
            except Exception as e:
                return Response(dict(message=str(e.args[0])), status=status.HTTP_400_BAD_REQUEST) 
        return Response(dict(message="data is not valid"), status=status.HTTP_400_BAD_REQUEST) 
...
"""
    API for CROP STAGE DETAILS & UPDATE & DELETE 
"""
class RFObservationDetailsV2View(APIView):
    """ 
        Purpose: details of pest 
    ...
    permission_classes = (IsAuthenticated,)
    queryset = FfwcImdRainfallObservation.objects.all()
    serializer_class = RFObsDetailsResSerializer 
    ...
    def get(self, request, id):
        ...
        req_serializer = RFObsReqSerializer(data=req_data)
        if req_serializer.is_valid():
            if len(FfwcImdRainfallObservation.objects.filter(st__id=id))==0:
                return Response(dict(message='Observation data doesnot exist'), status=status.HTTP_404_NOT_FOUND)
            queryset = FfwcImdRainfallObservation.objects.filter(st__id=id) 

            data = list(queryset.values())
            df = pd.DataFrame(data)
            print("inital df: ", df)
            
            df_non_zero = df[df['rainFall'] != 0]
            # print("df_non_zero: ", df_non_zero)
            ...
            print("date wise sort df: ", df)

            # df = df.drop_duplicates(subset=['date', 'rf_date'])
            df = df.drop_duplicates(keep='last')
            df.reset_index(drop=True, inplace=True)
            
            last_row_each_day = df[df['rf_date'].dt.time == pd.to_datetime('08:30:00').time()]
            # print("last_row_each_day: ", last_row_each_day)
            ...
            rf_id_list = df['id'].tolist()
            # print(rf_id_list)

            queryset = FfwcImdRainfallObservation.objects.filter(
                id__in=rf_id_list
            ).order_by('-rf_date') 

            # df_data = last_row_each_day.to_dict(orient='records')

            res_serializer = RFObsDetailsResV2Serializer(queryset, many=True)  
...
"""
    #####################################################################
    ### START Views.py for StationListViewSetV5
    #####################################################################
"""
from app_visualization.models import StationListDDDetailsV5
from app_visualization.observation_api.serializers import StationListDDDetailsV5Serializer
class StationListDDDetailsV5APIView(generics.ListAPIView):
    queryset = StationListDDDetailsV5.objects.all()
    serializer_class = StationListDDDetailsV5Serializer
"""
    #####################################################################
    ### END Views.py for StationListViewSetV5
    #####################################################################
"""
