...
SYSTEM_STATE_NAME_ECMWF_HRES = "IMD_GFS_VIS_DAILY"
...
# print("ECMWF_BASE_URL 001: ", ECMWF_BASE_URL)
ECMWF_BASE_URL = os.getcwd()
print("ECMWF_BASE_URL 002: ", ECMWF_BASE_URL)
max_workers = 10
...
class IMD_WRF_DATA:
    def ftp_connect(ftp_directory):
        ...
        with open(str(ECMWF_BASE_URL)+'/env_for_data_download_235.json', 'r') as envf:
            env_ = json.load(envf)
    ...
    def download_data(fdate, folder_path, filename):
        """
            ##################################################
            ...
            ##################################################
            ### Bash Command:
            ### wget --ftp-user=nwp --ftp-password=nwp -P /home/shaif/Downloads/2024062300 ftp://103.215.208.32/nwp-data/WRF_NETCDF/2024062300/WRF3km_INDIA-*.nc
            ##################################################
        """
        ...
        try:
            with yaspin(Spinners.dots, text="File Downloading...") as spinner:
                ...
                ftp = IMD_WRF_DATA.ftp_connect(ftp_directory)
                # print("################ IM HERE ###################")
    ...
    def merge_data(fdate, folder_path):
        """
            ##################################################
    ...
    def crop_nc_file_for_bd(fdate, folder_path):
        """
            ################################################################
    ...
    def main():
        # if len(sys.argv) != 2:
        #     print("wrong date input. please provide YYYYMMDD format")
        #     sys.exit(1)

        fcst_date = None
        
        try:
            fcst_date = sys.argv[1]
            print("Date argument:", fcst_date)
        ...
        WRF_NC_LOC_ECMWF_HRES = "/forecast/imd_wrf/"
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            list(tqdm(executor.map(lambda args: IMD_WRF_DATA.download_data(*args), download_args), total=len(download_args)))

        # self.download_data(date, folder_path)
        IMD_WRF_DATA.merge_data(date, folder_path)
        IMD_WRF_DATA.crop_nc_file_for_bd(date, folder_path)
