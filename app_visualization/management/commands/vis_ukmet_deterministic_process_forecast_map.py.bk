...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        print(f"### Generating forecast for date: {fdate}")

        source_obj = Source.objects.filter(
			name='UKMET_DETERMINISTIC_VIS', source_type="vis",
        ...
        # ncfile = str(ECMWF_BASE_URL)+str(WRF_NC_LOC_ECMWF_HRES)+f'{formatted_date_string}.nc'
        # path of 105
        # ncfile = f"/var/www/prod/ffwc_django/ffwc_django_project/UkMET_deterministic/ukmet_det_{fdate}/precip_{fdate}.nc"
        # path of 108
        ncfile = f"/home/rimes/ffwc-rebase/backend/ffwc_django_project/UkMET_deterministic/ukmet_det_{fdate}/precip_{fdate}.nc"
        ...
        for day in tqdm(range(6), desc="Processing Days"):
            start_step = day*1 
            end_step   = day*1 + 1 

            # start_time = dates[start_step].strftime('%Y%m%d%H')
            # end_time   = dates[end_step].strftime('%Y%m%d%H')
            start_time = dates[start_step].strftime('%Y%m%d')
            end_time   = dates[end_step].strftime('%Y%m%d')
            ...
            start_time_bst = (dt.strptime(start_time,'%Y%m%d')).strftime('%Y-%m-%d')
            end_time_bst   = (dt.strptime(end_time,'%Y%m%d')).strftime('%Y-%m-%d')
