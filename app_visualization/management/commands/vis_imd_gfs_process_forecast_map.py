# management/commands/vis_imd_gfs_process_forecast_map.py
...
SOURCE_NAME = 'IMD_GFS_VIS'
SYSTEM_STATE_NAME = app_visualization['system_state_name'][3]
IMD_GFS_BASE_URL = settings.BASE_DIR

class Command(BaseCommand):
    help = 'Generate geojson and svg colorbar legends for IMD GFS Forecast Maps'
    ...
    def handle(self, *args, **kwargs):
        ...
        ncfile = os.path.join(IMD_GFS_BASE_URL, WRF_NC_LOC.strip('/'), fdate, 'tp.nc')
        forecast_out_dir = os.path.join(IMD_GFS_BASE_URL, JSON_OUT_LOC.strip('/'), fdate)
        ...
        for day in tqdm(range(num_steps - 1), desc="Processing IMD GFS Map Layers"):
