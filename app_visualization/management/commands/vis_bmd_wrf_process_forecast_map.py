# management/commands/vis_bmd_wrf_process_forecast_map.py
...
class Command(BaseCommand):
    help = 'Generate forecast maps for BMDWRF'
    ...
    def handle(self, *args, **kwargs):
        ...
        if ui_date:
            # Clean incoming dashboard calendar template dashes safely: '2026-06-30' -> '20260630'
        ...
        WRX_NC_PATH_DIR = source_obj.source_path
        JSON_OUT_LOC = source_obj.destination_path

        date_obj = dt.strptime(fdate, '%Y%m%d')

        # Refactored manual string addition to clear os.path.join layouts
        ncfile = os.path.join(str(BMDWRF_BASE_URL), WRX_NC_PATH_DIR.strip("/"), f"wrf_out_{fdate}00.nc")
        forecast_out_dir = os.path.join(str(BMDWRF_BASE_URL), JSON_OUT_LOC.strip("/"), fdate)
        
        print(" $$$$$$$$$$$ ncfile: ", ncfile)
        print(" $$$$$$$$$$$ forecast_out_dir: ", forecast_out_dir)

        if not os.path.exists(ncfile):
            self.stdout.write(self.style.ERROR(f"❌ File not found: {ncfile}"))
            return
        ...
        finfo = {
            'fdate'  : fdate, 'rf': [], 'tmin': [], 'tmax': [],
            'rh': [], 'ws': [], 'cldflo': [], 'thi_min': [], 'thi_max': [],
        }

        # Dynamic array bound loop protection rules to prevent index crash parameters
        for day in tqdm(range(10), desc="Processing Days"):
            start_step = day * 8 
            end_step   = day * 8 + 8 

            if end_step >= len(dates):
                break
        ...
        # Commit telemetry update state record
