...
SYSTEM_STATE_NAME_ECMWF_HRES = app_visualization['system_state_name'][7]
...
class Command(BaseCommand):
	...
	def gen_upazila_forecast(
		...
		for idx, i_shape in enumerate(tqdm(shf)):
			...
			for day in range(3):
				...
				# if day == 9:
				# 	day_start_idx       = (day)*8
				# 	day_end_idx         = day_start_idx+6
				# else:
				day_start_idx       = (day)*24
				day_end_idx         = (day+1)*24
	...
	def main(self, date):
		...
		source_list = Source.objects.filter(
			name='IMD_WRF', source_type="basin_specific",
		...
		for my_source_details in source_list: 
			...
			nc_loc = str(BMDWRF_BASE_URL)+str(MY_ECMWF_HRES_DIR)+f'{date}00/{date}.nc'
			...
			# et0_obj = ET0CalculationConfiguration.objects.filter(source='IMD_WRF')[0]
			# et0_file_path = str(BASE_DIR) + str(et0_obj.destination_path) +  "IMD_WRF" + '/' + str(current_date_str) + '.nc'
