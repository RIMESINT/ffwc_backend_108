...
class Command(BaseCommand):
    help = 'Step 2: Generate Daily GeoJSON and SVG assets for IMD-GFS'
    ...
    def handle(self, *args, **kwargs):
        ...
        BASE_PATH = os.path.join(settings.BASE_DIR, 'assets', 'rainfall-anomaly', fdate, 'IMD-GFS')
        NC_FILE = os.path.join(BASE_PATH, f'imd_gfs_rainfall_anomaly_{fdate}.nc')

        if not os.path.exists(NC_FILE):
            self.stdout.write(self.style.ERROR(f"❌ Daily NetCDF not found: {NC_FILE}"))
            return

        ds = xr.open_dataset(NC_FILE)
        anomaly = ds['rainfall_anomaly']
        
        # Divergent Color Scale (Consistency across all sources)
        ...
        for i in tqdm(range(len(anomaly.time)), desc="Rasterizing IMD-GFS Daily"):
            data_slice = anomaly.isel(time=i)
            t_start_utc = pd.to_datetime(data_slice.time.values)
            s_str = t_start_utc.strftime('%Y%m%d%H')
            
            # BST Time display (+6 hours)
            # IMD-GFS usually starts at 03:00 UTC, so BST is 09:00 AM
            ...
            cb.set_label('IMD-GFS Daily Anomaly (mm/day)', fontsize=9, fontweight='bold')
            plt.savefig(os.path.join(BASE_PATH, svg_name), format='svg', bbox_inches='tight', transparent=True)
            plt.close(fig_cbar)

            finfo["rf"].append({"file": json_name, "cmap": svg_name, "start": start_bst, "end": end_bst})
        ...
        self.stdout.write(self.style.SUCCESS(f"✅ IMD-GFS Daily Assets Generated"))