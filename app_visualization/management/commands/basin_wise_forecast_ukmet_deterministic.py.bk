...
SYSTEM_STATE_NAME_ECMWF_HRES = app_visualization['system_state_name'][13]
...
class Command(BaseCommand):
	...
	def gen_upazila_forecast(
		...
		rf      = (ncf.variables['tp'][:])	#*1000  # meter to mm conversion
		...
		for idx, i_shape in enumerate(tqdm(shf)):
			...
			for day in range(6):
				...
				# print("source_obj: ", source_obj, ": "source_obj.name)
			...
			# print("upazila_data_daily: ", upazila_data_daily)
	...
	def main(self, date):
		print(" >>>>>>>>>>>>> Generating UKMET Deterministic Basin Wise Forecast for date: ", date)
		...
		source_list = Source.objects.filter(
			name='UKMET_DETERMINISTIC', source_type="basin_specific",
		...
		for my_source_details in source_list: 
			...
			print(" >>>>>>>>>>>>> source_obj : ", (source_obj))
			print(" >>>>>>>>>>>>> source_obj name: ", source_obj.name)
			print(" >>>>>>>>>>>>> source_obj id: ", source_obj.id)
			
			
			# nc_loc = str(BMDWRF_BASE_URL)+str(MY_ECMWF_HRES_DIR)+f'{nc_fmt_date}/tp.nc'
            # path of 105
			# nc_loc = f"/var/www/prod/ffwc_django/ffwc_django_project/UkMET_deterministic/ukmet_det_{date}/precip_{date}.nc"
            # path of 108
			nc_loc = f"/home/rimes/ffwc-rebase/backend/ffwc_django_project/UkMET_deterministic/ukmet_det_{date}/precip_{date}.nc"
			...
			# et0_obj = ET0CalculationConfiguration.objects.filter(source='UKMET_DETERMINISTIC')[0]
			# et0_file_path = str(BASE_DIR) + str(et0_obj.destination_path) +  "UKMET_DETERMINISTIC" + '/' + str(current_date_str) + '.nc'
			...
			for basin_details in basin_shape_file_path_obj:
				...
				if not basin_details.shape_file_path:
					print(f"!!! Skipping basin: {basin_details.name} (ID: {basin_details.id}) - No shapefile path.")
					continue
				...
				try:
					...
					print(f"Error processing {basin_details.name}: {e}")
