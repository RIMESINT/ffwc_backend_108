...
class Command(BaseCommand):
		...
		try:
			with yaspin(Spinners.dots, text="File Downloading...") as spinner:
				...
				if not os.path.exists(destination_directory):
					...
					os.chmod(destination_directory, stat.S_IRWXU | stat.S_IRWXG | stat.S_IRWXO)

				ftp_directory = f"/Pool_A/SFTP/UserHome/imdnwp/nwp_data/wrf_netcdf/{fdate}00/"
				...
				try:
					...
					files = sftp.listdir()
					for file in files:
						if file.startswith('WRF3km_INDIA-') and file.endswith('.nc'):
							local_path = os.path.join(destination_directory, file)
	...
	def main(self, date):
		...
		try:
			...
			# Filter only relevant NC files
			filenames = [f for f in filenames if f.startswith('WRF3km_INDIA-') and f.endswith('.nc')]
		except Exception as e:
		...
		with ThreadPoolExecutor(max_workers=max_workers) as executor:
			list(tqdm(
					executor.map(lambda args: self.download_data(*args), download_args), 
