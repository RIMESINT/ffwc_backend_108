...
SYSTEM_STATE_NAME_ECMWF_HRES = app_visualization['system_state_name'][5]
...
class Command(BaseCommand):
	...
	def main(self, date):
		...
		for my_source_details in source_list: 
			...
			nc_loc = str(BMDWRF_BASE_URL)+str(MY_ECMWF_HRES_DIR)+f'{date}.nc'
			...
			# et0_obj = ET0CalculationConfiguration.objects.filter(source='IMD_GFS')[0]
			# et0_file_path = str(BASE_DIR) + str(et0_obj.destination_path) +  "IMD_GFS" + '/' + str(current_date_str) + '.nc'
