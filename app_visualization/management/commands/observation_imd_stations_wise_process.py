...
from app_visualization.models import (
    Source, SystemState, FfwcIMDRFObservationStation,
	FfwcImdRainfallObservation,
...
class Command(BaseCommand):
	...
	def process_imd_stations_ARG_observations(self, fdate, state_list):
		...
		""" 
		print("##################################################")
		print(" Start ARG Station Processing")
		print("##################################################")
		try:
			with yaspin(Spinners.dots, text="ARG type station details storing...") as spinner:
				...
				for state in state_list:
					district_names = list(
						RainfallObservationIMDAWSDistricts.objects
						.filter(imd_aws_states_id__name=state)
						.order_by('id')
						.values_list('name', flat=True)
					)		
					# print("state: ", state)
					# print("district_names: ", district_names)
					# print("district_names type: ", type(district_names))
					# print("district_names len: ", len(district_names))
					...
					for district_name in district_names:
						...
						try: 
							...
							obs_url = f"http://aws.imd.gov.in:8091/AWS/tabulardataview.php?a=ARG&b={state}&c={district_name}&d=ALL_STATION&e={formatted_date}&f={formatted_date}&g=ALL_HOUR&h=ALL_MINUTE"
							print("obs_url: ", obs_url)
		
							response = session.get(obs_url, timeout=15)
							...
							df_rf = df_rf[[
								'S NO.', 'DISTRICT', 'STATION', 'DATE(YYYY-MM-DD)', 'TIME (IST)',
								'RAIN FALL CUM. SINCE 0830 IST (mm)'
							]]
							df_rf.rename(
								columns={
									'DATE(YYYY-MM-DD)': 'DATE',
									'TIME (IST)': 'TIMESTAMP',
									'RAIN FALL CUM. SINCE 0830 IST (mm)': 'ACC_RF',
								}, inplace=True
							)
							df_rf['STATE'] = state
							df_rf['NETWORK'] = "ARG"
				...
				combined_df_rf.head(100)
				# return

				x = 1
				for row in combined_df_rf.itertuples():
					# max_rf_id = FfwcImdRainfallObservation.objects.aggregate(
					# 	Max('rf_id')
					# )['rf_id__max']
					# print("max_rf_id: ", max_rf_id)

					st_obj = FfwcIMDRFObservationStation.objects.filter(
						name=row.STATION, 
						observe_data_source_network__name=row.NETWORK
					)
					if len(st_obj) == 0:
						continue
					else:
					...
					print("RF object insertion start .....")
					rf_obs_insert = FfwcImdRainfallObservation(
					...
					rf_obs_insert.save()
					print("RF object saved successfully")
	...
	def process_imd_stations_AWS_observations(self, fdate, state_list):
			...
			""" 
			print("##################################################")
			print(" Start AWS Station Processing")
			print("##################################################")
					...
					for state in state_list:
						...
						for district_name in district_names:
							...
							try:
								...
								obs_url = f"http://aws.imd.gov.in:8091/AWS/tabulardataview.php?a=AWS&b={state}&c={district_name}&d=ALL_STATION&e={formatted_date}&f={formatted_date}&g=ALL_HOUR&h=ALL_MINUTE"
								print("obs_url: ", obs_url)
	...
	def process_imd_stations_AGRO_observations(self, fdate, state_list):
			...
			""" 
			print("##################################################")
			print(" Start AGRO Station Processing")
			print("##################################################")

			try:
				with yaspin(Spinners.dots, text="AGRO type station details storing...") as spinner:
					...
					for state in state_list:
						...
						for district_name in district_names:
							...
							try:
								...
								obs_url = f"http://aws.imd.gov.in:8091/AWS/tabulardataview.php?a=AGRO&b={state}&c={district_name}&d=ALL_STATION&e={formatted_date}&f={formatted_date}&g=ALL_HOUR&h=ALL_MINUTE"
								print("obs_url: ", obs_url)
	...
	def main(self, date):
		...
		state_list = list(
			RainfallObservationIMDAWSStates.objects.order_by(
				'id'
			).values_list(
				'name', flat=True
			)
		...
		# print("state_list type: ", type(state_list))
