...
class Command(BaseCommand):
    help = 'Step 2: Generate Weekly GeoJSON and SVG assets for IMD-GFS'
    ...
    def handle(self, *args, **kwargs):
        ...
        NC_FILE = os.path.join(BASE_PATH, f"imd_gfs_weekly_anomaly_{fdate}.nc")

        if not os.path.exists(NC_FILE):
            self.stdout.write(self.style.ERROR(f"❌ Weekly NC not found: {NC_FILE}"))
            return

        ds = xr.open_dataset(NC_FILE)
        anomaly = ds['weekly_anomaly'].squeeze()
        
        # Weekly scales use tighter thresholds
        ...
        cb.set_label('IMD-GFS 7-Day Mean Anomaly (mm/day)', fontsize=9, fontweight='bold')
        ...
        self.stdout.write(self.style.SUCCESS(f"✅ IMD-GFS Weekly Assets Generated"))