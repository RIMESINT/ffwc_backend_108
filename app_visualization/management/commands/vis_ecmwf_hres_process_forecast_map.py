...
SOURCE_NAME = 'ECMWF_HRES_VIS'
SYSTEM_STATE_NAME = app_visualization['system_state_name'][1]
ECMWF_BASE_URL = settings.BASE_DIR

class Command(BaseCommand):
    help = 'Generate geojson and svg colorbar legends for ECMWF HRES Forecast'
    ...
    def handle(self, *args, **kwargs):
        ...
        ncfile = os.path.join(ECMWF_BASE_URL, WRF_NC_LOC.strip('/'), fdate, 'tp.nc')
        forecast_out_dir = os.path.join(ECMWF_BASE_URL, JSON_OUT_LOC.strip('/'), fdate)
        ...
        # Visualization Styling (Matching UKMET/BMD style)
        ...
        # dates_raw typically represents the T+24, T+48 marks
        dates_raw = num2date(time_var[:], time_var.units, time_var.calendar)
        # ECMWF 'tp' is in meters, multiply by 1000 for mm
        ...
        # Process 10 Days
        for day in tqdm(range(10), desc="Processing ECMWF Map Layers"):
            # logic: The NetCDF timestamp at index 'day' is the 'End' of the 24h accumulation.
            # We subtract 1 day to get the 'Start' label (Run Date).
            ...
            # ECMWF tp is accumulated from forecast start
            if day == 0:
                rf_d = rf[day, :, :]
            else:
                rf_d = rf[day, :, :] - rf[day-1, :, :]

            # Ensure no negative values from accumulation subtraction
        ...
        # Save Info JSON with formatting
