...
def import_experimental_forecast_files(self, duration, forecastDF_dict, stationNameToIdDict, station_name):
    """
    Celery task to import experimental water level forecast and observation data from a single DataFrame.
    Performs clean Delete-then-Insert operations to protect continuous historical timeseries metrics.
    """
    progress_recorder = ProgressRecorder(self)
    inserted_forecast_count = 0
    updated_forecast_count = 0  # Maintained at 0 due to clean replace operation layout
    inserted_observation_count = 0
    updated_observation_count = 0 # Maintained at 0 due to clean replace operation layout
    skipped_rows_count = 0
    total_initial_rows = 0

    logger.info(f"Task started for experimental station: {station_name}. Duration: {duration}")
    ...
    try:
        ...
        station_id_for_df = stationNameToIdDict.get(station_name)
        if station_id_for_df is None:
            error_msg = f"Station '{station_name}' not recognized inside master active dictionary lookups."
            logger.error(error_msg)
            progress_recorder.set_progress(100, 100, description=error_msg)
            return {"inserted_forecasts": 0, "inserted_observations": 0, "total_rows": 0, "state": states.SUCCESS, "message": error_msg}

        forecastDF['st_id'] = station_id_for_df

        # Identify experimental metrics based on design column suffixes
        min_col = next((col for col in forecastDF.columns.tolist() if col.endswith('-16')), None)
        max_col = next((col for col in forecastDF.columns.tolist() if col.endswith('-84')), None)
        mean_col = next((col for col in forecastDF.columns.tolist() if col.endswith('-Mean')), None)
        ...
        if not mean_col:
            error_msg = f"Missing essential experimental layout target column '{station_name}-Mean' inside file headers."
            logger.error(error_msg)
            progress_recorder.set_progress(100, 100, description=error_msg)
            return {"inserted_forecasts": 0, "inserted_observations": 0, "total_rows": 0, "state": states.SUCCESS, "message": error_msg}
        ...
        # Target naming translations for models mapping
        rename_map = {'YYYY-MM-DD HH:MM:SS': 'date_time', mean_col: 'waterlevel_mean'}
        if min_col: rename_map[min_col] = 'waterlevel_min'
        if max_col: rename_map[max_col] = 'waterlevel_max'
        if actual_observation_col: rename_map[actual_observation_col] = 'waterlevel_obs'
        ...
        total_rows_after_filter = len(forecastDF)
        if total_rows_after_filter == 0:
            logger.warning(f"No valid timeseries entries left to compute for station: {station_name}")
            progress_recorder.set_progress(100, 100, description="No valid forecast timelines detected.")
            return {"inserted_forecasts": 0, "inserted_observations": 0, "total_rows": total_initial_rows, "state": states.SUCCESS}
        ...
        for index, row in forecastDF.iterrows():
            ...
            try:
                ...
                normalized_dt = normalize_datetime(dt_obj_aware).replace(second=0)
            ...
            if index % 10 == 0:
                progress_recorder.set_progress(
                    20 + int((index + 1) * 20 / total_rows_after_filter),
                    100,
                    description=f"Parsed {index + 1}/{total_rows_after_filter} rows for {station_name}"
                )

        if not parsed_data:
            progress_recorder.set_progress(100, 100, description="No valid dataset rows parsed successfully.")
            return {"inserted_forecasts": 0, "inserted_observations": 0, "total_rows": total_initial_rows, "state": states.SUCCESS}

        total_processed_rows = len(parsed_data)
        progress_recorder.set_progress(40, 100, description=f"Executing wipe and replace transaction for {station_name}...")
    
        with transaction.atomic():
            station_ids_to_fetch = {data['station_id_val'] for data in parsed_data}
            station_objects_map = {s.station_id: s for s in Station.objects.filter(station_id__in=station_ids_to_fetch)}
            incoming_dates = [data['date_time'] for data in parsed_data]

            if incoming_dates:
                # 1. DELETE overlapping historical values across BOTH model classes atomically
                WaterLevelForecastsExperimentals.objects.filter(
                    station_id__station_id__in=station_ids_to_fetch,
                    forecast_date__in=incoming_dates
                ).delete()
                
                WaterLevelObservationExperimentals.objects.filter(
                    station_id__station_id__in=station_ids_to_fetch,
                    observation_date__in=incoming_dates
                ).delete()
                logger.info(f"Successfully purged overlapping experimental indices for station {station_name}")

            # 2. Map data fields into clean model instances
            forecast_instances = []
            ...
            # 3. INSERT the clean values via bulk operations
            if forecast_instances:
                WaterLevelForecastsExperimentals.objects.bulk_create(forecast_instances)
                inserted_forecast_count = len(forecast_instances)

            if observation_instances:
                WaterLevelObservationExperimentals.objects.bulk_create(observation_instances)
                inserted_observation_count = len(observation_instances)

            progress_recorder.set_progress(90, 100, description=f"Database synchronization finished for {station_name}.")

        result_message = (
            f"Synced {station_name}: Fresh Inserted {inserted_forecast_count} Forecasts "
            f"and {inserted_observation_count} Observations after clearing historical overlaps."
        )
        logger.info(result_message)
        ...
        logger.warning(f"Task for {station_name} ignored based on task constraints.")
        self.update_state(state=states.REVOKED, meta={'message': f"Task ignored.", 'percent': 100})
        return {"state": states.REVOKED, "message": "Bypassed execution."}

    except Exception as e:
        import traceback
        error_traceback = traceback.format_exc()
        logger.error(f"Task for {station_name} failed: {e}\n{error_traceback}")
        progress_recorder.set_progress(100, 100, description=f"Crash Error: {str(e)}")

        self.update_state(
            state='FAILED', 
            meta={
                'status': 'FAILED',
                'message': f"Critical internal crash: {str(e)}", 
                'percent': 100
            }
        )
        return {"inserted_forecasts": 0, "inserted_observations": 0, "total_rows": 0, "state": "FAILED", "message": str(e)}
