...
class Command(BaseCommand):
    help = 'Fetch Cumilla ensemble forecast supporting manual UI date selections with rolling fallback'

    def add_arguments(self, parser):
        # 1. Positional argument support for direct console execution and crontab macros
        parser.add_argument('fdate', nargs='?', type=str, help='Forecast target date in YYYYMMDD format')
    ...
    def handle(self, *args, **options):
        # Determine parameter overrides dynamically
        ...
        # Explicitly build execution date folder sequence: Target Date -> Previous Day Fallback
        target_fdate = base_datetime.strftime('%Y%m%d')
        fallback_fdate = (base_datetime - timedelta(days=1)).strftime('%Y%m%d')

        self.stdout.write(self.style.NOTICE(f"Initializing Cumilla Forecast Update Core | Target Window: {target_fdate} | Fallback Day-1: {fallback_fdate}"))

        # Fire pipeline ingestion; fallback automatically if the primary directory fails
        ...
        if not success:
            ...
            if not success_fallback:
                self.stderr.write(self.style.ERROR(f"❌ Critical Failure: Cumilla ensemble data missing across both primary and fallback options."))
    ...
    def execute_ingestion_pipeline(self, date_folder_str):
        ...
        # Use safe temporary filesystem paths
        ...
        try:
            ...
            # --- STEP 1: Attempt Ingestion from NEW Directory structure using current loop date ---
            ...
            # If both paths return blank results for this specific directory, abort to trigger fallback inside handle() loop
            if not remote_file_path:
            ...
            # Filter indices starting from the execution start date folder
            ...
            pb_dates, pb_values = [], []
            pb_name = f"exceedence_{date_folder_str}_cumilla.csv"
            ...
            try:
                ...
                self.stdout.write(self.style.WARNING("--> Probability exceedence file not present inside active remote date directory."))
            ...
            self.stdout.write(self.style.SUCCESS(f"DONE: Cumilla updated successfully for run parameter date: {formatted_date}."))
            return True
