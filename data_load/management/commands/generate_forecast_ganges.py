...
class Command(BaseCommand):
    help = 'Fetch Ganges ensemble forecast and probability data'

    def handle(self, *args, **options):
        now = datetime.now()
        run_datetime = now.strftime('%Y-%m-%d %H:%M:%S')
        self.stdout.write(f"[{run_datetime}] Starting Ganges forecast update...")
        ...
        try:
            ...
            # Lookups chronological order: Today -> Yesterday -> Fallback Directory Scan
            ...
            # 5. Process Probability File from webdat (e.g., 20260621_b.pb.csv)
            pb_dates, pb_values = [], []
            ...
            # 6. Save Formatted Output JSON (station_id: "101")
            output_data = {
            ...
            self.stdout.write(self.style.SUCCESS(f"DONE: Ganges updated for {formatted_date}."))
