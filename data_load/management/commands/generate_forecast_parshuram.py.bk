...
class Command(BaseCommand):
    help = 'Scan available YYYYMMDD folders for Parshuram and fetch the most recent valid forecast'
        ...
        try:
            ...
            for folder in date_folders:
                ...
                try:
                    ...
                    self.stdout.write(self.style.SUCCESS(f"Found valid Parshuram data in: {folder}"))
                    break
                except IOError:
                    continue
            ...
            # 5. Download
            temp_csv = f'/tmp/latest_parshuram_temp.csv'
            sftp.get(remote_file_path, temp_csv)
            sftp.close()
            ssh.close()

            # 6. Data Processing (WITH TIMEZONE FIX)
            df = pd.read_csv(temp_csv)
            time_col = 'Time' if 'Time' in df.columns else 'Date'
            
            # Convert to datetime and strip timezone info to prevent comparison errors
            ...
            self.stdout.write(self.style.SUCCESS(f'Successfully updated Parshuram JSON from folder {final_date_folder}'))

        except Exception as e:
            self.stderr.write(self.style.ERROR(f'Critical Error for Parshuram: {str(e)}'))