...
class Command(BaseCommand):
    help = 'Fetch Sylhet ensemble forecast supporting manual UI date parameters with historical rollback'
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.NOTICE(f"Initializing Sylhet Forecast Update Core | Target Window: {target_fdate} | Fallback Day-1: {fallback_fdate}"))
        ...
        if not success:
            ...
            if not success_fallback:
                self.stderr.write(self.style.ERROR(f"❌ Critical Failure: Sylhet ensemble data missing across both primary and fallback options."))
    ...
    def execute_ingestion_pipeline(self, date_folder_str):
        ...
        OLD_ROOT = '/home/mmb/Tank/outputs_corrected/sylhet/csv/'
        TARGET_FILE = 'all_en_sylhet_corr.csv'
        ...
        fd1, temp_csv = tempfile.mkstemp(suffix='_sylhet_main.csv')
        fd2, temp_pb_csv = tempfile.mkstemp(suffix='_sylhet_pb.csv')
        ...
        try:
            ...
            pb_dates, pb_values = [], []
            pb_name = f"exceedence_{date_folder_str}_sylhet.csv"
            ...
            self.stdout.write(self.style.SUCCESS(f"DONE: Sylhet updated successfully for run parameter date: {formatted_date}."))
            return True
