...
class Command(BaseCommand):
    help = 'Fetch Sunamganj ensemble forecast supporting manual UI date parameters with historical rollback'
    ...
    def handle(self, *args, **options):
        # Capture parameter inputs cleanly across available slots
        ...
        # Explicitly configure fallback path options: Selected Target, then Yesterday
        target_fdate = base_datetime.strftime('%Y%m%d')
        fallback_fdate = (base_datetime - timedelta(days=1)).strftime('%Y%m%d')

        self.stdout.write(self.style.NOTICE(f"Initializing Sunamganj Forecast Update Core | Target Window: {target_fdate} | Fallback Day-1: {fallback_fdate}"))

        # Fire pipeline ingestion; if target folder fails, roll back to yesterday automatically
        ...
        if not success:
            ...
            if not success_fallback:
                self.stderr.write(self.style.ERROR(f"❌ Critical Failure: Sunamganj ensemble data missing across both primary and fallback options."))
    ...
    def execute_ingestion_pipeline(self, date_folder_str):
        ...
        OLD_ROOT = '/home/mmb/Tank/outputs_corrected/sunamganj/csv/'
        TARGET_FILE = 'all_en_sunamganj_corr.csv'
        ...
        # Generate temporary files for localized file handling
        ...
        try:
            ...
            # --- STEP 1: Attempt Ingestion from NEW Directory structure using current parameter date folder ---
            ...
            # If both paths return blank results for this folder, abort to trigger fallback inside handle() loop
            if not remote_file_path:
            ...
            # Filter data to start from the target execution folder date
            ...
            pb_dates, pb_values = [], []
            pb_name = f"exceedence_{date_folder_str}_sunamganj.csv"
            ...
            try:
                ...
                if 'ex_pr' in df_pb.columns:
                    ...
                    self.stdout.write(self.style.SUCCESS(f"--> Probability exceedence data loaded."))
            ...
            self.stdout.write(self.style.SUCCESS(f"DONE: Sunamganj updated successfully for run parameter date: {formatted_date}."))
            return True
