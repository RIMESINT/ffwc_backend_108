...
class Command(BaseCommand):
    help = 'Fetch Sunamganj ensemble forecast (5% to 95%) with nested fallback (New Path -> Old Path)'
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if not remote_file_path:
                raise Exception("Data not found in either NEW or OLD Sunamganj directory structures.")
            ...
            # 4. Probability Computation (data_pb)
            pb_dates, pb_values = [], []
            pb_name = f"exceedence_{final_date_folder}_sunamganj.csv"
            ...
            self.stdout.write(self.style.SUCCESS(f"DONE: Sunamganj updated for {formatted_date}."))
