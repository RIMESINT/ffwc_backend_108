...
class Command(BaseCommand):
    ...
    def save_to_database(self, results, prediction_date, basin_id):
        rows = []
        h_map = {idx: v[0] for idx, v in STATION_THRESHOLDS[basin_id].items()}
        t_map = {idx: v[1] for idx, v in STATION_THRESHOLDS[basin_id].items()}
        for d_key, vals in results.items():
            for idx, v in vals.items():
                rows.append(UKMetPreMonsoonBasinWiseFlashFloodForecast(
                    prediction_date=prediction_date, basin_id=basin_id,
                    date=datetime.strptime(d_key, "%Y-%m-%d").date(),
                    hours=h_map[idx], thresholds=t_map[idx], value=v
                ))
        if rows:
