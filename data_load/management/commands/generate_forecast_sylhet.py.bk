...
class Command(BaseCommand):
    help = 'Scan available folders for Sylhet, fetch the most recent valid forecast, and filter out historical data'
    ...
    def handle(self, *args, **options):
        ...
        # Base directory and filename for Sylhet
        remote_base_dir = '/home/mmb/Tank/outputs_corrected/sylhet/csv/'
        target_filename = 'all_en_sylhet_corr.csv'
        ...
        try:
            ...
            self.stdout.write("Scanning Sylhet remote directories...")
            all_entries = sftp.listdir(remote_base_dir)
            
            # Filter for YYYYMMDD folders and sort descending (newest first)
            ...
            # 4. Loop back through folders until the CSV is found
            for folder in date_folders:
                candidate_path = f"{remote_base_dir}{folder}/{target_filename}"
                try:
                    sftp.stat(candidate_path)  # Verify file exists
            ...
            # 8. Save/Overwrite the JSON file
            with open(local_json_path, 'w') as f:
                json.dump(output_data, f, indent=2)

            self.stdout.write(self.style.SUCCESS(f'Successfully updated latest_sylhet_forecast.json from folder {final_date_folder}'))
