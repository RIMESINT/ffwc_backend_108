...
class Command(BaseCommand):
    help = 'Fetch Amalshid ensemble forecast with nested fallback (New Path -> Old Path)'
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if not remote_file_path:
                raise Exception("Data not found in either NEW or OLD Amalshid directory structures.")
            ...
            pb_dates, pb_values = [], []
            pb_name = f"exceedence_{final_date_folder}_amalshid_s.csv"
            ...
            self.stdout.write(self.style.SUCCESS(f"DONE: Amalshid updated for {formatted_date}."))
