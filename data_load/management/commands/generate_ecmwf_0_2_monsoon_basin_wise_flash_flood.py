...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        # 1. Support positional arguments for direct cron tasks or manual console executions
        parser.add_argument('fdate', nargs='?', type=str, help='Target date in format YYYYMMDD or YYYY-MM-DD')
    ...
    def handle(self, *args, **options):
        ...
        date_input = raw_date or datetime.today().strftime('%Y-%m-%d')
        ...
        self.stdout.write(self.style.SUCCESS(f"🚀 Starting ECMWF Pre-Monsoon Run: {standard_date}"))
        high_risk_summary = []
    ...
    def compute_forecast_mean(self, stationGDF, dt_obj):
        ...
        file_path = ""
        # Cascade Fallback: Check targeted run date, then step back 1 day if missing
        for offset in [0, 1]:
            check_date = dt_obj - timedelta(days=offset)
            file_name = check_date.strftime('%d%m%Y') + ".nc"
            p = os.path.join(forecast_dir, file_name)
            if os.path.exists(p):
                file_path = p
                break
