...
# Target Model
from data_load.models import Ecmwf_Pre_Monsoon_Probabilistic_Flash_Flood_Forecast
    ...
    def calculate_exceedance_probability(self, all_member_rainfall, thresholds, given_date, station_name, trace):
        ...
        for p_date in forecast_range:
            ...
            for idx, values in thresholds.items():
                ...
                if trace:
                    self.stdout.write(
                        f"   {hour}h Window | Thresh: {threshold:6.2f}mm | "
                        f"Mean Rain: {mean_rainfall:6.2f}mm | Prob: {prob:6.2f}% ({exceed_count}/{num_members})"
                    )
    ...
    def save_to_database(self, results, prediction_date, basin_id):
        ...
        for date_key, values in results.items():
            ...
            for idx_str, prob_val in values.items():
                rows.append(Ecmwf_Pre_Monsoon_Probabilistic_Flash_Flood_Forecast(
        ...
        if rows:
            Ecmwf_Pre_Monsoon_Probabilistic_Flash_Flood_Forecast.objects.filter(prediction_date=prediction_date, basin_id=basin_id).delete()
