...
class Command(BaseCommand):
    help = 'Generates ECMWF Probabilistic Flash Flood Forecasts using the Probabilistic_Flash_Flood_Forecast model.'
    ...
    def handle(self, *args, **kwargs):
        ...
        if not self.run_forecast_for_date(date_std):
        ...
        try:
            with xr.open_dataset(filepath) as ds:
                ...
                for d, v in daily_incremental.items():
                    # Day-0 recovery: Attribute end-of-day 00:00 morning slice to the run date
            ...
            print(f"      ⚠️ Error processing member {filename}: {e}")
            return None
    ...
    def save_to_database(self, results, prediction_date, basin_id):
        ...
        for date_key, values in results.items():
            ...
            for idx_str, prob_val in values.items():
                # Correct Model usage: Probabilistic_Flash_Flood_Forecast
                rows.append(Probabilistic_Flash_Flood_Forecast(
                    prediction_date=prediction_date, 
                    basin_id=basin_id,
                    date=datetime.strptime(date_key, "%Y-%m-%d").date(),
                    hours=h_meta.get(idx_str), 
                    thresholds=t_meta.get(idx_str), 
                    value=prob_val
                ))
        if rows:
            Probabilistic_Flash_Flood_Forecast.objects.filter(
                prediction_date=prediction_date, 
                basin_id=basin_id
            ).delete()
            Probabilistic_Flash_Flood_Forecast.objects.bulk_create(rows)

    def run_forecast_for_date(self, date_input):
        self.stdout.write(self.style.SUCCESS(f'--- ECMWF ENS Calculation Run: {date_input} ---'))
        success = False
        ...
        for basin_id, station_name in STATION_DICT.items():
            ...
            all_members_data = []
            # ECMWF 51 Members (EN00-EN50)
            for i in range(51):
                fname = f"{date_nodash}_EN{str(i).zfill(2)}.nc"
