...
from app_water_watch_mobile.rf_level_input.serializers import (
...
# class RFLevelStationForMobileUserViewSet(generics.ListAPIView):
#     """
#         Read-only endpoint that returns the WaterWatchRFLevelStationForMobileUser
...
#         return (
#             WaterWatchRFLevelStationForMobileUser.objects
...
class RFLevelStationForMobileUserViewSet(generics.ListAPIView):
    """
        GET /api/my-stations/  -> list of WaterWatchRFLevelStationForMobileUser
    ...
    queryset = WaterWatchRFLevelStationForMobileUser.objects.none()
    ...
    def get_queryset(self):
        ...
        qs = WaterWatchRFLevelStationForMobileUser.objects.filter(
            mobile_user=user,
            is_active=True
        )
...
class BulkRFLevelCreateAPIView(APIView):
    """
    ...
    def post(self, request, *args, **kwargs):
        ...
        if valid_objs:
            with transaction.atomic():
                ...
                created_instances = RFLevelInputForMobileUser.objects.bulk_create(instances)
...
class Last7DaysRFLevelAPIView(generics.ListAPIView):
    """
    ...
    def get_queryset(self):
        ...
        return (
            RFLevelInputForMobileUser.objects.filter(
                created_by=user,
...
class RFLevelUpdateAPIView(generics.UpdateAPIView):
    """
        PUT endpoint to update a RFLevelInputForMobileUser by id using QuerySet.update()
    """
    ...
    def get_object(self):
        ...
        try:
            obj = RFLevelInputForMobileUser.objects.select_related('created_by', 'station').get(pk=pk)
        except RFLevelInputForMobileUser.DoesNotExist:
        ...
        has_access = obj.station and WaterWatchRFLevelStationForMobileUser.objects.filter(
            mobile_user=user,
    ...
    def put(self, request, *args, **kwargs):
        ...
        with transaction.atomic():
            update_count = RFLevelInputForMobileUser.objects.filter(pk=instance.pk).update(
        ...
        updated_obj = RFLevelInputForMobileUser.objects.select_related(
...
class DeleteRFLevelInputAPIView(APIView):
    """
    ...
    def delete(self, request, pk, *args, **kwargs):
        ...
        obj = get_object_or_404(RFLevelInputForMobileUser, pk=pk)
        ...
        mapping_exists = WaterWatchRFLevelStationForMobileUser.objects.filter(
            mobile_user=user,
